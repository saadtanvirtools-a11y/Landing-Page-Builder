...
async function fetchTemplateFromFirestore(templateId: string): Promise<Template | null> {
  try {
    ...
    console.error("[PreviewPage] Firestore fetch error:", err);
    return null;
  }
}
...
export default function PreviewPage() {
  ...
  const { currentTemplate, loadFromStorage, loadTemplate } = useEditorStore();
  ...
  const templateId: string | null = routeTemplateId ?? user?.assignedTemplateId ?? currentTemplate?.id ?? null;
  ...
  // ensure preview has template even on refresh/direct entry
  useEffect(() => {
    let cancelled = false;

    const init = async () => {
      // If store already has correct template, use it
      if (currentTemplate?.id && templateId && currentTemplate.id === templateId) {
      ...
      if (user?.id) {
        try {
          ...
          console.error("[PreviewPage] loadFromStorage error:", e);
        }
      }
  ...
  }, [templateId, user?.id, currentTemplate?.id, loadFromStorage, loadTemplate]);

  const blockCount = currentTemplate?.blocks?.length ?? 0;
  const hasContent = status === "ready" && blockCount > 0;

  // write processed HTML into iframe
  useEffect(() => {
    const iframe = iframeRef.current;
    if (!iframe || !currentTemplate?.rawHtml || status !== "ready") return;
    ...
    doc.write(finalHtml);
    doc.close();
  }, [currentTemplate?.rawHtml, device, status]);

  const handleBackToEditor = () => {
    navigate("/editor", {
      state: { templateId: currentTemplate?.id ?? templateId ?? null },
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* Top Bar */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-gray-900 text-white px-4 py-2.5 flex items-center justify-between gap-4 shadow-lg">
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={handleBackToEditor}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-gray-800 hover:bg-gray-700 rounded-lg transition border border-gray-700"
          >
      ...
      <div className="flex-1 flex items-start justify-center pt-14 pb-8 px-4">
        {status === "loading" && (
          <div className="flex flex-col items-center justify-center min-h-[calc(100vh-56px)] text-center px-6">
            ...
            <h2 className="text-xl font-bold text-gray-700 mb-2">Loading preview...</h2>
            <p className="text-gray-400 text-sm">Preparing your latest saved page</p>
          </div>
        )}
