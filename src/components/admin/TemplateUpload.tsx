...
interface ValidationCheck {
  label: string; ok: boolean; blocking: boolean; message: string; detail?: string;
}
interface ValidationResult {
  passed: boolean; checks: ValidationCheck[]; warnings: number; errors: number;
}
...
interface ExistingTemplate {
  id: string; templateName: string; category: string; rawHtml?: string; createdAt?: string;
}
...
function validateHtmlStructure(htmlString: string): ValidationCheck[] {
  ...
  checks.push({
    label: "Has data-block sections", ok: blocks.length > 0, blocking: true,
    message: blocks.length > 0
      ? `Found ${blocks.length} block${blocks.length !== 1 ? "s" : ""}: ${blockIds.slice(0, 5).join(", ")}${blockIds.length > 5 ? "…" : ""}`
  ...
  checks.push({
    label: "No duplicate block IDs", ok: duplicates.length === 0, blocking: true,
  ...
  checks.push({
    label: "Has data-editable fields", ok: editables.length > 0, blocking: true,
  ...
  checks.push({
    label: "All editables have data-editable-type", ok: missingType.length === 0, blocking: true,
    message: missingType.length === 0
  ...
  const invalidTypes = editables.filter((e) => { const t = e.getAttribute("data-editable-type"); return t && !validTypes.includes(t); });
  checks.push({
    label: "Valid data-editable-type values", ok: invalidTypes.length === 0, blocking: true,
  ...
  checks.push({
    label: "Has CSS variables in :root", ok: cssVarNames.length > 0, blocking: true,
    message: cssVarNames.length > 0
  ...
  if (colorVarEls.length > 0) {
    checks.push({
      label: "data-color-vars reference valid CSS vars", ok: brokenColorVars.length === 0, blocking: false,
      message: brokenColorVars.length === 0
...
async function validateZip(file: File): Promise<{ result: ValidationResult; htmlString: string | null }> {
  ...
  checks.push({ label: "File format is .zip", ok: file.name.endsWith(".zip"), blocking: true, message: file.name.endsWith(".zip") ? "Valid ZIP file ✓" : "File must be a .zip archive" });
  const sizeMB = file.size / (1024 * 1024);
  checks.push({ label: "File size under 10MB", ok: sizeMB < 10, blocking: true, message: sizeMB < 10 ? `${sizeMB.toFixed(2)} MB — OK ✓` : `${sizeMB.toFixed(2)} MB — exceeds 10MB limit` });
  ...
  try {
    ...
    checks.push({ label: "Contains index.html", ok: !!htmlFile, blocking: true, message: htmlFile ? `Found: ${htmlFile} ✓` : "index.html is missing from ZIP" });
    checks.push({ label: "Contains public/style.css", ok: !!cssFile, blocking: true, message: cssFile ? `Found: ${cssFile} ✓` : "public/style.css is missing" });
    if (htmlFile) { htmlString = await zip.files[htmlFile].async("string"); checks.push(...validateHtmlStructure(htmlString)); }
  } catch {
  ...
  return { result: { passed: blockingFailed.length === 0, checks, warnings, errors: blockingFailed.length }, htmlString };
}
...
function CheckRow({ check }: { check: ValidationCheck }) {
  ...
  return (
    <div className={`rounded-lg border text-xs overflow-hidden transition-all ${check.ok ? "border-green-100 bg-green-50/50" : check.blocking ? "border-red-100 bg-red-50/50" : "border-amber-100 bg-amber-50/50"}`}>
      <div className="flex items-start gap-2 px-3 py-2">
        {check.ok ? <CheckCircle size={13} className="text-green-500 mt-0.5 shrink-0" /> : isWarning ? <AlertTriangle size={13} className="text-amber-500 mt-0.5 shrink-0" /> : <XCircle size={13} className="text-red-400 mt-0.5 shrink-0" />}
...
export default function TemplateUpload({ onUploaded }: Props) {
  ...
  // Live name duplicate check against Firestore
  useEffect(() => {
    if (!templateName.trim()) { setLiveNameDup(null); return; }
  ...
  async function doUpload(overrideFileCheck = false, overrideNameCheck = false) {
    ...
    if (!overrideFileCheck) {
      ...
      if (contentDup) { setFileDup(contentDup); return; }
    }
    if (!overrideNameCheck) {
      const nameDup = await findDuplicateByName(templateName);
      if (nameDup) { setLiveNameDup(nameDup); return; }
    }
    ...
    try {
      ...
      (parsedTemplate as any).rawHtml = htmlString;

      setUploadState({ status: "uploading", message: "Saving to Firestore...", progress: 80 });

      // ✅ Save to Firestore instead of localStorage
      await setDoc(doc(db, "templates", templateId), parsedTemplate);
  ...
  function handleUpload() { doUpload(fileOverride, nameOverride); }

  function handleReset() {
    setSelectedFile(null); setTemplateName(""); setValidation(null);
    setShowAllChecks(false); setParsedHtml(null); setFileDup(null);
    setFileOverride(false); setNameOverride(false); setLiveNameDup(null);
    setUploadState({ status: "idle", message: "", progress: 0 });
  ...
  const canUpload =
    !!selectedFile && !!templateName.trim() && validation?.passed === true &&
    !isUploading && !isValidating && uploadState.status !== "success" &&
    !(!!fileDup && !fileOverride) && !(!!liveNameDup && !nameOverride);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-violet-600">
        ...
        <p className="text-xs text-indigo-200 mt-0.5">ZIP must contain a properly annotated <code className="bg-white/20 px-1 rounded">index.html</code></p>
      </div>

      <div className="p-6 space-y-5">
        {/* Template Name */}
        <div>
          <label className="text-xs font-bold text-gray-600 block mb-1.5 uppercase tracking-wide">Template Name <span className="text-red-500">*</span></label>
          <input type="text" value={templateName} onChange={(e) => setTemplateName(e.target.value)} placeholder="e.g. Modern SaaS Landing"
            className={`w-full px-3 py-2.5 rounded-xl border text-sm outline-none bg-gray-50 text-gray-800 transition-all
              ${liveNameDup && !nameOverride ? "border-amber-300 focus:border-amber-400 focus:ring-2 focus:ring-amber-100" : "border-gray-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"}`} />
        </div>
            ...
            <div className="flex items-start gap-3 px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-1.5 px-3 py-2 bg-red-100 rounded-lg border border-red-200">
                  ...
                  <p className="text-xs text-red-500">{fileDup.category} · {fileDup.createdAt ?? "previously"}</p>
                </div>
              </div>
            </div>
            <div className="flex gap-2 px-4 pb-3">
              <button type="button" onClick={() => { setFileDup(null); setSelectedFile(null); setValidation(null); setParsedHtml(null); setFileOverride(false); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                className="flex-1 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-xs font-bold transition">🗂 Choose Different File</button>
              <button type="button" onClick={() => { setFileOverride(true); setFileDup(null); }}
                className="flex-1 py-2 rounded-lg border-2 border-red-400 text-red-600 hover:bg-red-100 text-xs font-bold transition">⚠️ Save as New Copy</button>
            </div>
          </div>
        )}
          ...
          <div className="bg-amber-50 border-2 border-amber-400 rounded-xl overflow-hidden">
            <div className="flex items-start gap-3 px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-700 mt-0.5">A template named <span className="font-bold">"{liveNameDup.templateName}"</span> already exists.</p>
              </div>
            </div>
            <div className="flex gap-2 px-4 pb-3">
              <button type="button" disabled={!selectedFile || !validation?.passed || isUploading}
                onClick={() => { setNameOverride(true); doUpload(fileOverride, true); }}
              ...
              <button type="button" onClick={() => { setTemplateName(""); setLiveNameDup(null); }}
                className="flex-1 py-2 rounded-lg border-2 border-amber-400 text-amber-700 hover:bg-amber-100 text-xs font-bold transition">✏️ Rename Template</button>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <svg className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2.5"><polyline points="6 9 12 15 18 9" /></svg>
          </div>
        </div>
        ...
        <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={onDrop}
          ...
          className={`rounded-xl border-2 border-dashed p-8 flex flex-col items-center gap-3 cursor-pointer transition-all duration-200
            ${dragOver ? "border-indigo-500 bg-indigo-50 scale-[0.99]"
              : fileDup && !fileOverride ? "border-red-400 bg-red-50"
              : selectedFile ? validation?.passed ? "border-green-300 bg-green-50" : validation && !validation.passed ? "border-red-300 bg-red-50" : "border-indigo-300 bg-indigo-50"
          ...
          {isValidating ? (
            <><Loader2 size={36} className="text-indigo-400 animate-spin" /><p className="text-sm font-semibold text-indigo-600">Validating...</p></>
          ) : selectedFile ? (
            <>
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${validation?.passed ? "bg-green-100" : validation ? "bg-red-100" : "bg-indigo-100"}`}>
                {validation?.passed ? <CheckCircle size={28} className="text-green-500" /> : validation ? <XCircle size={28} className="text-red-400" /> : <FileArchive size={28} className="text-indigo-500" />}
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-gray-700">{selectedFile.name}</p>
                <p className="text-xs text-gray-400 mt-0.5">{(selectedFile.size / (1024 * 1024)).toFixed(2)} MB
                  {validation && <span className={`ml-2 font-semibold ${validation.passed ? "text-green-600" : "text-red-500"}`}>· {validation.passed ? `✓ Valid${validation.warnings > 0 ? ` (${validation.warnings} warning${validation.warnings !== 1 ? "s" : ""})` : ""}` : `${validation.errors} error${validation.errors !== 1 ? "s" : ""}`}</span>}
                </p>
              </div>
            ...
            <>
              <div className="w-14 h-14 rounded-2xl bg-gray-100 flex items-center justify-center"><Upload size={28} className="text-gray-300" /></div>
              <div className="text-center">
        ...
        {(isValidating || validation) && (
          <div className="rounded-xl border border-gray-200 overflow-hidden">
            <div className={`px-4 py-2.5 flex items-center justify-between ${validation?.passed ? "bg-green-50 border-b border-green-100" : validation ? "bg-red-50 border-b border-red-100" : "bg-gray-50 border-b border-gray-100"}`}>
              <div className="flex items-center gap-2"><Info size={13} className="text-gray-400" /><span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Validation Report</span></div>
              {validation && (
                <div className="flex items-center gap-2">
                  {validation.errors > 0 && <span className="text-xs bg-red-100 text-red-600 font-bold px-2 py-0.5 rounded-full">{validation.errors} error{validation.errors !== 1 ? "s" : ""}</span>}
                  {validation.warnings > 0 && <span className="text-xs bg-amber-100 text-amber-600 font-bold px-2 py-0.5 rounded-full">{validation.warnings} warning{validation.warnings !== 1 ? "s" : ""}</span>}
            ...
            <div className="p-3 space-y-1.5 bg-white">
              {isValidating ? (
                <div className="flex items-center gap-2 text-xs text-gray-400 py-2"><Loader2 size={13} className="animate-spin text-indigo-400" /> Running validation checks...</div>
              ) : (
                <>
                  {visibleChecks.map((c, i) => <CheckRow key={i} check={c} />)}
                  {sortedChecks.length > 5 && (
                    <button onClick={() => setShowAllChecks(!showAllChecks)} className="w-full text-xs text-indigo-500 hover:text-indigo-700 font-semibold py-1.5 flex items-center justify-center gap-1">
                      {showAllChecks ? <><ChevronUp size={12} /> Show less</> : <><ChevronDown size={12} /> Show {sortedChecks.length - 5} more checks</>}
                    </button>
                  )}
                </>
              )}
        ...
        {isUploading && (
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs"><span className="text-gray-500 font-medium">{uploadState.message}</span><span className="text-indigo-600 font-bold">{uploadState.progress}%</span></div>
        ...
        {uploadState.status === "success" && (
          <div className="flex items-start gap-3 p-4 rounded-xl bg-green-50 border border-green-200">
            ...
            <div><p className="text-sm font-bold text-green-700">Upload Successful!</p><p className="text-xs text-green-600 mt-0.5">{uploadState.message}</p></div>
          </div>
        )}
        {uploadState.status === "error" && (
          <div className="flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
            <AlertCircle size={18} className="text-red-400 mt-0.5 shrink-0" />
            <div><p className="text-sm font-bold text-red-600">Upload Failed</p><p className="text-xs text-red-500 mt-0.5">{uploadState.message}</p></div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          <button onClick={handleUpload} disabled={!canUpload}
            ...
            {isUploading ? <><Loader2 size={15} className="animate-spin" /> Processing...</> : fileDup && !fileOverride ? <><XCircle size={15} /> Blocked — Duplicate File</> : liveNameDup && !nameOverride ? <><AlertTriangle size={15} /> Blocked — Duplicate Name</> : <><Upload size={15} /> Upload Template</>}
          </button>
          <button onClick={handleReset} className="px-4 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors">Reset</button>
        </div>

        {/* ZIP Guide */}
        <div className="rounded-xl border border-amber-100 bg-amber-50 overflow-hidden">
          <div className="px-3 py-2 border-b border-amber-100 flex items-center gap-2"><AlertTriangle size={13} className="text-amber-500" /><p className="text-xs font-bold text-amber-700">Required ZIP structure</p></div>
