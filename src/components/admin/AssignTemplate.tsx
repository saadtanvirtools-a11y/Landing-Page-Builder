...
interface Props { selectedTemplate: Template | null; }
...
export default function AssignTemplate({ selectedTemplate }: Props) {
  ...
  async function handleAssign(userId: string) {
    ...
    try {
      ...
      setUsers((prev) => prev.map((u) =>
        u.id === userId
          ? { ...u, assignedTemplateId: selectedTemplate.id, assignedTemplateName: selectedTemplate.templateName }
          : u
      ));

      // ✅ Update localStorage user if it's the currently logged-in user
      ...
      setFlashMsg({ type: "success", text: `"${selectedTemplate.templateName}" assigned to ${userName}!` });
    } catch {
  ...
  async function handleRemove(userId: string) {
    ...
    try {
      ...
      setUsers((prev) => prev.map((u) =>
        u.id === userId ? { ...u, assignedTemplateId: null, assignedTemplateName: null } : u
      ));
      ...
      if (currentUser) {
        const parsed = JSON.parse(currentUser) as User;
        if (parsed.id === userId) {
          localStorage.setItem("user", JSON.stringify({ ...parsed, assignedTemplateId: null, assignedTemplateName: null }));
          useAuthStore.getState().loadFromStorage();
        }
      }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-5">
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl bg-indigo-50 border border-indigo-100">
        <div className="w-9 h-9 rounded-lg bg-indigo-100 flex items-center justify-center shrink-0"><LayoutTemplate size={16} className="text-indigo-600" /></div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-indigo-700 truncate">{selectedTemplate.templateName}</p>
          <p className="text-xs text-indigo-400">{selectedTemplate.blocks.length} blocks · ID: {selectedTemplate.id}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Assigned",     value: users.filter((u) => u.assignedTemplateId).length, color: "#22c55e" },
          { label: "Unassigned",   value: users.filter((u) => !u.assignedTemplateId).length, color: "#f59e0b" },
        ].map((s) => (
      ...
      {flashMsg && (
        <div className="flex items-center gap-2 p-3 rounded-xl text-xs font-medium border"
          style={{ backgroundColor: flashMsg.type === "success" ? "#f0fdf4" : "#fef2f2", borderColor: flashMsg.type === "success" ? "#bbf7d0" : "#fecaca", color: flashMsg.type === "success" ? "#166534" : "#991b1b" }}>
          ...
          {filteredUsers.length === 0 ? (
            ...
            return (
              <div key={user.id} className="flex items-center gap-3 p-3 rounded-xl border transition-all"
                style={{ borderColor: isAssignedThis ? "#a5b4fc" : "#f3f4f6", backgroundColor: isAssignedThis ? "#eef2ff" : "#fafafa" }}>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {isAssignedThis ? (
                    <button onClick={() => handleRemove(user.id)} disabled={isRemoving}
                      ...
                      {isRemoving ? <><Loader2 size={11} className="animate-spin" /> Removing...</> : <><XCircle size={11} /> Remove</>}
                    </button>
                  ) : (
                    <button onClick={() => handleAssign(user.id)} disabled={isAssigning}
                      ...
                      {isAssigning ? <><Loader2 size={11} className="animate-spin" /> Assigning...</> : <><UserCheck size={11} /> {isAssignedOther ? "Reassign" : "Assign"}</>}
                    </button>
                  )}
                </div>
              </div>
