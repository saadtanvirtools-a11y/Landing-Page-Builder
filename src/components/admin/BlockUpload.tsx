...
// ✅ Simple fast string hash (no crypto needed)
...
// ✅ Check if this exact file content was already uploaded
function findDuplicateByContent(htmlContent: string): StandaloneBlock | null {
  try {
    const incoming = hashString(htmlContent.trim());
    const existing: StandaloneBlock[] = JSON.parse(localStorage.getItem("lp_blocks") || "[]");
    return existing.find((b) => hashString((b.rawHtml || "").trim()) === incoming) ?? null;
  } catch {
    return null;
  }
}

// Name-based duplicate check (still used for submit guard)
function findDuplicateByName(name: string, type: string): StandaloneBlock | null {
  try {
    const existing: StandaloneBlock[] = JSON.parse(localStorage.getItem("lp_blocks") || "[]");
    return existing.find((b) => b.blockName.trim().toLowerCase() === name.trim().toLowerCase() && b.blockType === type) ?? null;
  } catch {
    return null;
  }
}
...
function extractCSSVariables(doc: Document): CSSVariables {
  ...
  for (const tag of Array.from(doc.querySelectorAll("style"))) {
...
function extractEditables(blockEl: Element): EditableItem[] {
  return Array.from(blockEl.querySelectorAll("[data-editable]")).map((el) => {
    ...
    const content = type === "image" ? (el as HTMLImageElement).src || el.getAttribute("src") || "" : (el.textContent || "").trim();
...
function parseBlockHtml(htmlString: string): {
  ...
  return { cssVariables: extractCSSVariables(doc), editables: extractEditables(blockEl), rawHtml: htmlString };
}
...
function saveBlock(block: StandaloneBlock): void {
  const existing: StandaloneBlock[] = JSON.parse(localStorage.getItem("lp_blocks") || "[]");
  existing.push(block);
  localStorage.setItem("lp_blocks", JSON.stringify(existing));
}
...
export default function BlockUpload({ onUploaded }: Props) {
  ...
  // Name duplicate (detected at submit time)
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      // ✅ CHECK FILE CONTENT DUPLICATE RIGHT HERE — before even parsing
      const contentDup = findDuplicateByContent(htmlString);
      if (contentDup) {
        setFileDup(contentDup);
        // Still parse so admin can see the file info — but block save
        ...
        return; // Don't proceed — show the file duplicate warning
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // ✅ File content duplicate guard (synchronous fresh check)
    if (!fileOverride) {
      const contentDup = findDuplicateByContent(htmlContent);
      if (contentDup) {
        setFileDup(contentDup);
    ...
    // ✅ Name duplicate guard (synchronous fresh check)
    if (!nameOverride) {
      const liveDup = findDuplicateByName(blockName, blockType);
      if (liveDup) {
        setError(`A block named "${liveDup.blockName}" (${liveDup.blockType}) already exists.`);
        return;
      }
    }
  ...
  async function handleFileOverride() {
  ...
  const liveDupByName = findDuplicateByName(blockName, blockType);
  const submitDisabled = uploading || !htmlContent || !!parseError || (!!fileDup && !fileOverride) || (!!liveDupByName && !nameOverride);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ✅ FILE CONTENT duplicate warning — shown immediately after file is picked */}
        ...
        <div className="bg-red-50 border-2 border-red-400 rounded-xl overflow-hidden">
          <div className="flex items-start gap-3 px-4 py-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-red-500 mt-2">Upload a different file, or click below to save it as a new separate copy anyway.</p>
            </div>
          </div>
          <div className="flex gap-2 px-4 pb-3">
            ...
            <button type="button" onClick={handleFileOverride} className="flex-1 py-2 rounded-lg border-2 border-red-400 text-red-600 hover:bg-red-100 text-xs font-bold transition">
              ⚠️ Save as New Copy
            </button>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="border border-indigo-100 rounded-xl overflow-hidden">
            <div className="px-4 py-2.5 bg-indigo-50 border-b border-indigo-100 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setAllowedUserIds(users.map((u) => u.id))} className="text-xs text-indigo-500 hover:text-indigo-700 font-semibold">
                  All
                </button>
                <span className="text-indigo-200">|</span>
                <button type="button" onClick={() => setAllowedUserIds([])} className="text-xs text-indigo-500 hover:text-indigo-700 font-semibold">
                  None
                </button>
              </div>
            </div>
        ...
        <div>
            ...
            {fileDup && !fileOverride ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-400">
                  ZIP must contain an <code className="bg-gray-100 px-1 rounded">index.html</code> with <code className="bg-gray-100 px-1 rounded">data-block</code> attribute
                </p>
              </div>
            )}
          </div>
          <div className="mt-2 p-3 bg-blue-50 border border-blue-100 rounded-lg">
            ...
            <ul className="text-xs text-blue-600 space-y-0.5 list-disc list-inside">
              ...
              <li>
                CSS variables defined in <code className="bg-blue-100 px-1 rounded">:root {"{}"}</code>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={handleReset} className="px-5 py-3 border border-gray-200 text-gray-500 hover:bg-gray-50 font-semibold rounded-xl text-sm transition">
            Reset
          </button>
        </div>
      </form>
