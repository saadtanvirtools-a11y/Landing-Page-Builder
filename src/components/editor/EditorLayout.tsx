...
import { isPanelDrag, parseDragId, isStandaloneDrag, parseStandaloneDragId } from "../../components/editor/BlockPanel";
import type { Template, StandaloneBlock } from "../../types";

// ─────────────────────────────────────────────────────────────
// Drag overlay card
// ─────────────────────────────────────────────────────────────
function DragOverlayCard({ option, standaloneBlock }: { option: BlockOption | null; standaloneBlock: StandaloneBlock | null }) {
  if (option) {
    return (
      <div
        className="w-48 rounded-xl border-2 border-indigo-500 bg-white
                      shadow-2xl px-3 py-2.5 rotate-1 opacity-95 pointer-events-none"
      >
  ...
  if (standaloneBlock) {
    return (
      <div
        className="w-48 rounded-xl border-2 border-green-500 bg-white
                      shadow-2xl px-3 py-2.5 rotate-1 opacity-95 pointer-events-none"
      >
...
// ─────────────────────────────────────────────────────────────
// Build grouped template blocks
// ✅ NOW accepts assignedTemplateId — filters to only that template
// ─────────────────────────────────────────────────────────────
function buildGroupedBlocks(LABELS: Record<string, string>, assignedTemplateId: string | null): Record<string, BlockOption[]> {
  let allTemplates: Template[] = [];
  try {
    const all: Template[] = JSON.parse(localStorage.getItem("lp_templates") || "[]");
    // ✅ Filter: regular user sees only their assigned template
    // null = admin, sees all
    allTemplates = assignedTemplateId ? all.filter((t) => t.id === assignedTemplateId) : all;
  } catch {}
  ...
  allTemplates.forEach((template) => {
    template.blocks.forEach((block) => {
      ...
      const label = LABELS[block.blockId] ?? block.blockId;
...
export default function EditorLayout() {
  ...
  // ✅ Get assigned template ID from logged-in user
  ...
  // ✅ Ensure currentTemplate matches assignedTemplateId
  useEffect(() => {
    if (assignedTemplateId && currentTemplate?.id !== assignedTemplateId) {
      try {
        const raw = localStorage.getItem("lp_templates");
        if (raw) {
          const templates: Template[] = JSON.parse(raw);
          const found = templates.find((t) => t.id === assignedTemplateId);
          if (found) {
            loadTemplate(found);
          }
        }
      } catch (err) {
        console.error("[EditorLayout] Failed to load assigned template:", err);
      }
    }
  ...
  const canvasBlockIds = useMemo(
    () =>
      (currentTemplate?.blocks ?? []).map((b) => {
        if (b.sourceTemplateId?.startsWith("standalone::")) {
          return b.sourceTemplateId;
        }
        return b.blockId + "::" + b.sourceTemplateId;
      }),
  ...
  const handleDragStart = useCallback(
    (e: DragStartEvent) => {
      ...
      if (isPanelDrag(id)) {
        // ✅ Pass assignedTemplateId so only correct template blocks are found
        const groups = buildGroupedBlocks(BLOCK_LABELS, assignedTemplateId);
      ...
      if (isStandaloneDrag(id)) {
        try {
          ...
          const allBlocks: StandaloneBlock[] = JSON.parse(localStorage.getItem("lp_blocks") || "[]");
          const found = allBlocks.find((b) => b.id === standaloneId);
    ...
    [updateDropZone, assignedTemplateId],
  );
  ...
  const handleDragEnd = useCallback(
    (e: DragEndEvent) => {
      ...
      const doAdd = (parsedBlock: ReturnType<typeof standaloneToParseBlock> | any) => {
      ...
      if (isPanelDrag(activeId)) {
        // ✅ Pass assignedTemplateId here too
      ...
      if (isStandaloneDrag(activeId)) {
        ...
        try {
          ...
          console.error("[DragEnd] ❌ Failed to parse standalone blocks:", err);
        }
      }
    },
    [addBlockToTemplate, updateDropZone, assignedTemplateId],
  );
  ...
  const buildExportHtml = (template: Template): string => {
    ...
    if (!cssVarStyle) {
      cssVarStyle = doc.createElement("style");
      ...
      <div className="flex flex-col h-screen overflow-hidden bg-gray-50">
        ...
        <nav
          className="bg-white border-b border-gray-100 px-4 py-3
                        flex items-center justify-between flex-shrink-0 z-10 shadow-sm"
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {currentTemplate ? `${currentTemplate.blocks.length} section${currentTemplate.blocks.length !== 1 ? "s" : ""}` : "Drag blocks from the left panel to start"}
              </p>
            </div>
          </div>

          <div className="text-xs text-gray-400 hidden md:block">{isSaving ? "💾 Saving..." : lastSaved ? `✅ Saved ${new Date(lastSaved).toLocaleTimeString()}` : "⬜ Not saved yet"}</div>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* ✅ Pass assignedTemplateId so BlockPanel filters correctly */}
          <BlockPanel draggingOption={draggingOption} canvasBlockIds={canvasBlockIds} assignedTemplateId={assignedTemplateId} />
